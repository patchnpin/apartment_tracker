...
try: 
    ...
    if os.path.exists(details_path):
        details_df = pd.read_csv(details_path, dtype={'Unit Number': str})
    else:
        details_df = pd.DataFrame(columns=[
            'Unit Number', 'Floor Plan', 'Building', 'SqFt',
            'Bedrooms', 'Bathrooms', 'Floor', 'Garage', 'Fireplace', 'Lake View'
        ])
    ...
    price_df = pd.concat([price_df, pd.DataFrame(new_prices)], ignore_index=True)
    price_df.to_csv(price_path, index=False)
    print(f"Added {len(new_prices)} new price entries\n")

    # ── Update Unit Details (upsert) ─────────────────────────────────
